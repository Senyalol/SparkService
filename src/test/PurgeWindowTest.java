...
class PurgeWindowTest {
    ...
    void shouldRemoveEntriesOlderThanWindow() {
        ...
        entries.add(new SparkStreamingApp.TransactionEntry(now - 600000, 100.0, "Deposit")); // 10 min ago
        entries.add(new SparkStreamingApp.TransactionEntry(now - 120000, 200.0, "Deposit")); // 2 min ago
        entries.add(new SparkStreamingApp.TransactionEntry(now - 30000, 300.0, "Deposit"));  // 0.5 min ago

        SparkStreamingApp.purgeWindow(entries, now, WINDOW_MS);

        assertEquals(2, entries.size(), "Should have 2 entries after purge");

        // Проверяем, что все оставшиеся записи не старше окна
        for (SparkStreamingApp.TransactionEntry entry : entries) {
    ...
    void shouldKeepEntriesWithinWindow() {
        long now = System.currentTimeMillis();
        entries.add(new SparkStreamingApp.TransactionEntry(now - 60000, 100.0, "Deposit"));   // 1 min ago
        entries.add(new SparkStreamingApp.TransactionEntry(now - 120000, 200.0, "Deposit"));  // 2 min ago
    ...
    void shouldHandleEmptyList() {
        SparkStreamingApp.purgeWindow(entries, System.currentTimeMillis(), WINDOW_MS);
    ...
    void shouldHandleBoundary() {
        long now = System.currentTimeMillis();
        entries.add(new SparkStreamingApp.TransactionEntry(now - WINDOW_MS, 100.0, "Deposit"));
    ...
    void shouldRemoveAllWhenAllAreOlder() {
        long now = System.currentTimeMillis();
        entries.add(new SparkStreamingApp.TransactionEntry(now - 600000, 100.0, "Deposit"));
        entries.add(new SparkStreamingApp.TransactionEntry(now - 700000, 200.0, "Deposit"));
        entries.add(new SparkStreamingApp.TransactionEntry(now - 800000, 300.0, "Deposit"));
    ...
    void shouldKeepAllWhenAllAreWithinWindow() {
        long now = System.currentTimeMillis();
        entries.add(new SparkStreamingApp.TransactionEntry(now - 60000, 100.0, "Deposit"));
        entries.add(new SparkStreamingApp.TransactionEntry(now - 120000, 200.0, "Deposit"));
        entries.add(new SparkStreamingApp.TransactionEntry(now - 180000, 300.0, "Deposit"));
